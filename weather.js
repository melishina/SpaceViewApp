class GetWeatherAPI {
	constructor(targetEl, options) {
		...
		if (!options) {
			...
			const apiRequest = `https://cors.5apps.com/?uri=http://api.openweathermap.org/data/2.5/forecast/daily?q=${this
				.location}&units=${this.units}&APPID=${this.appid}&cnt=${this
				.forecastDays}`;
	...
	getUserLocation() {
		$.getJSON('https://ipinfo.io/json', data => {
			...
			const apiRequest = `https://cors.5apps.com/?uri=http://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&units=metric&APPID=${this
				.appid}&cnt=${this.forecastDays}`;
	...
	getWeather(apiRequest, locData) {
		...
		// apiRequest = `https://cors.5apps.com/?uri=d9q3wbd9wqybosx`;
		...
		var tryWeather = () => {
			$.getJSON(apiRequest, data => {
				...
				if (retries > 0) {
					console.log('trying weather #', retries);
					retries--;
					setTimeout(tryWeather, 2000);
				} else {
					console.log(`Error: ${errorThrown}`);
					console.log(`TextStatus: ${textStatus}`);
					if (status == 'timeout') {
						$('.weatherCard1').html('<p>Refresh page</p>');
					}
				}
			});
		};
		...
		let forecast = nextDays
			.map((nextDay, i) => {
				return `<div class="forecastCard"><u><p>${nextDay}</p></u>
                  <p class="forecastMax">Max ${data.list[
										i + 1
									].main.temp_max.toFixed(0)}°C</p>
                  <p class="forecastMin">Min ${data.list[
										i + 1
									].main.temp_min.toFixed(0)}°C</p></div>`;
			})
			.join('');
